...
if __name__ == '__main__':
    ...
    for dataset in content :
        ...
        config.Data.splitting = 'FileBased'
        config.Data.unitsPerJob = 1
        config.Data.outputPrimaryDataset = dataset.split('/')[1]
	#site=sites[n%4]
   ...
   #		config.Data.outLFNDirBase = '/store/user/%s/NanoTestProd006/' % (getUsernameFromSiteDB())
	...
	n+=1
	nnn="%s"%n
        config.General.requestName = "VHbbPostNano2016_V2_March22_LSpWA_"+dataset.split('/')[1][:30]+dataset.split('/')[2][:30]+nnn
        config.Data.outputDatasetTag = dataset.split('/')[2][:30]+nnn+"_1File"
