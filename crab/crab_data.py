...
config.General.requestName = 'VHbbPostNano2016_V1'
...
config.JobType.inputFiles = ['../keep_and_drop.txt','../postproc_data.py','../../../../../../scripts/haddnano.py'] #hadd nano will not be needed once nano tools are in cmssw
...
config.Data.outLFNDirBase = '/store/user/%s/VHbbPostNano2016_V1/' % (getUsernameFromSiteDB())
...
if __name__ == '__main__':
   ...
   #		config.Data.outLFNDirBase = '/store/user/%s/NanoTestProd006/' % (getUsernameFromSiteDB())
	...
	n+=1
	nnn="%s"%n
        config.General.requestName = "VHbbPostNano2016_V1"+dataset.split('/')[1][:30]+dataset.split('/')[2][:30]+nnn
